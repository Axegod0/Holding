...
export default function SwapOfferModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-950/85 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-lg bg-gray-900 border border-amber-500/50 rounded-3xl shadow-[0_0_50px_rgba(245,158,11,0.25)] overflow-hidden flex flex-col relative">
        ...
        <div className="p-6 space-y-4 max-h-[400px] overflow-y-auto">
          <p className="text-xs font-mono text-gray-300">
            <strong className="text-amber-400">👑 {activeSwapOffer.fromName}</strong> size çift taraflı bir takas teklif etti.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-2xl bg-emerald-950/20 border border-emerald-500/30 space-y-3">
              ...
              <div className="space-y-2">
                <div>
                  <span className="text-[9px] font-mono text-gray-500 block">Nakit Para:</span>
                  <span className="text-lg font-mono font-black text-emerald-400">
                    +{activeSwapOffer.offeredCash?.toLocaleString('tr-TR')} ₺
                  </span>
                </div>
                <div>
                  <span className="text-[9px] font-mono text-gray-500 block">Mülk(ler):</span>
                  <span className="text-xs font-mono text-white font-bold leading-relaxed">
        ...
        {/* Alt Butonlar */}
        <div className="p-6 pt-2 border-t border-gray-800/60 flex items-center gap-3">
          <button
            onClick={() => respondSwapOffer(activeSwapOffer.id, false)}
            disabled={loading}
            className="flex-1 py-3.5 px-4 rounded-xl bg-gray-800 hover:bg-red-950/50 text-gray-300 hover:text-red-300 font-mono font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all cursor-pointer border border-gray-700 hover:border-red-500/40 disabled:opacity-50"
          >
