...
export default function TradeHubPanel() {
  ...
  if (!ownsRawMaterial && !ownsFactory && !ownsMall) {
    return (
      <div className={`w-full border rounded-3xl p-6 shadow-md space-y-5 text-center animate-fade-in ${
        isLight ? 'bg-white border-amber-400 text-slate-900 shadow-amber-500/10' : 'bg-gray-900/90 border-amber-500/30 text-white shadow-[0_0_35px_rgba(245,158,11,0.12)]'
      }`}>
        ...
        <div>
          ...
          <p className={`text-xs font-mono max-w-md mx-auto mt-1 ${isLight ? 'text-slate-500' : 'text-gray-400'}`}>
            Üretim ve ticaret halkasına katılmak için tahtadaki 3 kritik tesisten (ID 15, 24 veya 35) en az birini satın almalısınız.
          </p>
        </div>
        <div className={`grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 text-left text-xs font-mono ${isLight ? 'text-slate-700' : 'text-gray-300'}`}>
          <div className={`p-3 rounded-xl border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-gray-950/60 border-gray-800'}`}>
            <span className="text-amber-400 font-bold block mb-1">#15 Maden & Tarım</span>
            Her tur otomatik +1 birim hammadde üretir. Fabrikalara satılır veya devlete ihraç edilir.
          </div>
          <div className={`p-3 rounded-xl border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-gray-950/60 border-gray-800'}`}>
            <span className="text-amber-400 font-bold block mb-1">#24 Üretim Fabrikası</span>
            Hammaddeyi işleyerek Son Ürün'e çevirir. AVM'lere satılır veya devlete ihraç edilir.
          </div>
          <div className={`p-3 rounded-xl border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-gray-950/60 border-gray-800'}`}>
            <span className="text-amber-400 font-bold block mb-1">#35 Mega AVM & Perakende</span>
  ...
  return (
    <div className={`w-full border rounded-3xl p-6 space-y-6 ${
      isLight ? 'bg-white border-amber-400 text-slate-900 shadow-md' : 'bg-gray-900/90 border-amber-500/30 text-white shadow-[0_0_35px_rgba(245,158,11,0.12)]'
    }`}>
      
      {/* Panel Başlığı */}
      <div className={`flex items-center justify-between border-b pb-4 ${isLight ? 'border-slate-200' : 'border-gray-800'}`}>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gray-950 border border-gray-800">
          ...
          <span className="text-xs font-mono text-gray-400">Bakiye:</span>
          <span className="text-sm font-mono font-bold text-white">{myState.balance?.toLocaleString('tr-TR')} ₺</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {ownsRawMaterial ? (
          <div className="p-5 rounded-2xl bg-gray-950/80 border border-yellow-500/40 space-y-4 flex flex-col justify-between relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-yellow-500/5 rounded-full blur-2xl pointer-events-none" />
            
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-mono font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg bg-yellow-500/15 text-yellow-400 border border-yellow-500/30">
                  1. HALKA - HAMMADDE
                </span>
                <span className="text-xs font-mono text-gray-400">ID #15</span>
              </div>
              ...
              <div className="p-3 rounded-xl bg-gray-900 border border-gray-800 flex items-center justify-between">
                <span className="text-xs font-mono text-gray-400">Stok Miktarı:</span>
            ...
            <div className="space-y-3 pt-3 border-t border-gray-800">
              <button
                ...
                className="w-full py-2.5 px-3 rounded-xl bg-gray-800 hover:bg-emerald-950/40 text-emerald-300 font-mono text-xs font-bold flex items-center justify-center gap-2 border border-gray-700 hover:border-emerald-500/40 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <ShieldCheck className="w-4 h-4" />
                <span>Devlete Sat (+20.000 ₺)</span>
              </button>

              <div className="p-3 rounded-xl bg-gray-900/90 border border-gray-800 space-y-2.5">
                <div className="text-[11px] font-mono text-gray-300 font-bold flex items-center gap-1">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <select
                    ...
                    className="w-full bg-gray-950 border border-gray-700 rounded-lg p-2 text-xs text-white font-mono focus:outline-none focus:border-yellow-500"
                  >
          ...
          <div className="p-5 rounded-2xl bg-gray-950/40 border border-gray-800/60 flex flex-col items-center justify-center text-center p-8 text-gray-500 space-y-2">
        ...
        {ownsFactory ? (
          <div className="p-5 rounded-2xl bg-gray-950/80 border border-slate-400 space-y-4 flex flex-col justify-between relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-slate-500/5 rounded-full blur-2xl pointer-events-none" />
            
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-mono font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg bg-slate-500/20 text-slate-300 border border-slate-500/40">
                  2. HALKA - FABRİKA
                </span>
                <span className="text-xs font-mono text-gray-400">ID #24</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2.5 rounded-xl bg-gray-900 border border-gray-800">
                  <span className="text-[10px] font-mono text-gray-400 block">Hammadde Depo:</span>
                  <span className="text-sm font-mono font-bold text-yellow-400">
                    📦 {tradeState[24]?.rawMaterialStock || 0}
                  </span>
                </div>
                <div className="p-2.5 rounded-xl bg-gray-900 border border-gray-800">
                  <span className="text-[10px] font-mono text-gray-400 block">Bitmiş Ürün:</span>
            ...
            <div className="space-y-2.5 pt-3 border-t border-gray-800">
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="py-2 px-2.5 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-300 font-mono text-[11px] font-bold flex items-center justify-center gap-1 border border-gray-700 transition-all cursor-pointer disabled:opacity-40"
                ...
                <button
                  ...
                  className="py-2 px-2.5 rounded-xl bg-gray-800 hover:bg-emerald-950/40 text-emerald-300 font-mono text-[11px] font-bold flex items-center justify-center gap-1 border border-gray-700 hover:border-emerald-500/40 transition-all cursor-pointer disabled:opacity-40"
              ...
              <div className="p-3 rounded-xl bg-gray-900/90 border border-gray-800 space-y-2.5">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <select
                    ...
                    className="w-full bg-gray-950 border border-gray-700 rounded-lg p-2 text-xs text-white font-mono focus:outline-none focus:border-cyan-500"
                  >
        ...
        {ownsMall ? (
          <div className="p-5 rounded-2xl bg-gray-950/80 border border-pink-500/50 space-y-4 flex flex-col justify-between relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-pink-500/5 rounded-full blur-2xl pointer-events-none" />
            
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-mono font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg bg-pink-500/20 text-pink-400 border border-pink-500/40">
                  3. HALKA - MEGA AVM
                </span>
                <span className="text-xs font-mono text-gray-400">ID #35</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2.5 rounded-xl bg-gray-900 border border-gray-800">
                  <span className="text-[10px] font-mono text-gray-400 block">Depo Ürün Stoku:</span>
                  <span className="text-sm font-mono font-bold text-cyan-400">
                    🏬 {tradeState[35]?.productStock || 0} Adet
                  </span>
                </div>
                <div className="p-2.5 rounded-xl bg-gray-900 border border-gray-800">
                  <span className="text-[10px] font-mono text-gray-400 block">Vitrin Dolu Süre:</span>
                  <span className={`text-sm font-mono font-bold ${(tradeState[35]?.activeStockTurns || 0) > 0 ? 'text-pink-400' : 'text-gray-500'}`}>
              ...
              <div className={`p-3 rounded-xl border text-xs font-mono font-bold flex items-center justify-center gap-2 text-center ${
                (tradeState[35]?.activeStockTurns || 0) > 0
                  ? 'bg-pink-500/15 border-pink-500/40 text-pink-300 shadow-lg shadow-pink-500/10'
            ...
            <div className="space-y-3 pt-3 border-t border-gray-800">
              <button
                ...
                className="w-full py-2.5 px-3 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-300 font-mono text-xs font-bold flex items-center justify-center gap-2 border border-gray-700 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
              >
